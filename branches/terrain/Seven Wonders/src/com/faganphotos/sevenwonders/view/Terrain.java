 ...
 * Terrain for a level.
 * 
 * Stored as an image in the assets folder.
 * The brightness of each pixel indicates Y position in the game.
 * White is the highest, black the lowest.
 * The X,Y axes of the image correspond to the X,Z axes of the game, respectively.
 */
public class Terrain {
	 ...
	 * Create terrain.
	 * 
	 * @param file String path to image file with the elevations to use
	 * @param xStartParam float x coordinate where terrain should start in game
	 * @param xEnd float x coordinate where terrain should end in game
	 * @param yStartParam float y coordinate where terrain should start in game
	 * @param yEnd float y coordinate where terrain should end in game
	 * @param zStartParam float z coordinate where terrain should start in game
	 * @param zEnd float z coordinate where terrain should end in game
	 */
	public Terrain(String fileParam, 
		final float xStartParam, final float xEnd, 
		final float yStartParam, final float yEnd, 
		final float zStartParam, final float zEnd) {
	 ...
	 * @param texture SubTexture to use for each two triangles added
	 * @param density int number of vertexes to break X and Z axes up into
	 * @param geometry OpenGLGeometryBuilder to add to
	 */
	public void addToGeometry(
		final Context context,
		final SubTexture texture,
		final int density,
		final OpenGLGeometryBuilder<GeometryBuilder.TexturableTriangle3D<GeometryBuilder.NormalizableTriangle3D<Void>>, GeometryBuilder.TexturableRectangle2D<Void>> geometry) {
			...
			for( int xCount = 1; xCount < density; xCount++ ) {			
				...
				geometry
					...
					.setTextureCoordinates(
						texture.s1, texture.t1, 
						texture.s1, texture.t2,
						texture.s2, texture.t1
					).setNormal(
	 ...
	 * 
	 * @param p1x float x coordinate of the first point in the triangle
	 * @param p1y float y coordinate of the first point in the triangle
	 * @param p1z float z coordinate of the first point in the triangle
	 * @param p2x float x coordinate of the second point in the triangle
	 * @param p2y float y coordinate of the second point in the triangle
	 * @param p2z float z coordinate of the second point in the triangle
	 * @param p3x float x coordinate of the third point in the triangle
	 * @param p3y float y coordinate of the third point in the triangle
	 * @param p3z float z coordinate of the third point in the triangle
	 * @param normal float[3] to reuse for storing the normal in
	 * @return float array with normal
	 */
	private static float[] calculateNormal(
		final float p1x, final float p1y, final float p1z,
		final float p2x, final float p2y, final float p2z,
		final float p3x, final float p3y, final float p3z,
		float[] normal) {
		...
		final float length = FloatMath.sqrt(
			normal[0] * normal[0] 
			+ normal[1] * normal[1] 
			+ normal[2] * normal[2]
		);		
	 ...
	 * 
	 * @param numerator int numerator of fraction of range traversed
	 * @param denominator int denominator of fraction of range traversed
	 * @param start float lowest number in the range
	 * @param run float length of the range
	 ...
	 * 
	 * @param imageX int x location of the pixel
	 * @param imageY int y location of the pixel
	 * @param heightStart float minimum height
	 * @param heightRun float height range
	 * @return height
	 */
