...
public class PlayActivity extends Activity {
    ...
    private Handler updateUiHandler = new Handler() {
    	...
    	if(latestRemainingTimeSeconds<0){Toast toast1=Toast.makeText(getApplicationContext(), "TIME OUT !!!", Toast.LENGTH_SHORT);
        toast1.setGravity(Gravity.CENTER, 0, 0);
        LinearLayout toastView = (LinearLayout) toast1.getView();
        ImageView imageCode = new ImageView(getApplicationContext());
        imageCode.setImageResource(R.drawable.history);
        toastView.addView(imageCode, 0);
                      toast1.show();
                      finish();}
    	};
    };
